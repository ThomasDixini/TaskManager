...
- [ ] Opening the dialog with a given `Task` pre-fills Title, Description, Project selection, and Priority selection to match that task's current values.
- [ ] Changing fields and clicking Save calls `TaskService.update` with the new values and closes the dialog on success; the task's entry in `TaskService.tasks()` reflects the changes afterward (verifiable since the board, once built, reads that same signal).
- [ ] The Project dropdown lists all projects from `ProjectService.projects()` by name, plus an option to add a new project inline.
- [ ] Using the inline "add new project" flow creates a project via `ProjectService.create`, and the new project becomes available and selected in the dropdown without closing the task editor.
- [ ] The Priority dropdown offers Low, Medium, High, and a way to select "no priority" (`null`).
- [ ] Clicking Delete requires a confirmation step before actually calling `TaskService.delete`; confirming deletes the task and closes the dialog.
- [ ] The Save button is disabled (or shows a validation error) when the Title field is empty.
- [ ] No compile errors; `ng build` succeeds with this component added (it doesn't need to be routed/rendered anywhere yet — that's task 601's job).
